...
$row_ch = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM `workshop_author` WHERE ( `username` = '" . $_POST['username'] . "' )"));
if ($row_ch > 0) { ?>
    <script>
...
<?php
    ...
    if ($fname != "" && $title != "" && $lname != "" && $username != "" && $password != "" && $identify != "" && $welcome != "" && $welcome != "" && $exbanquet != "" && $organization != "" && $section != "" && $country != "" && $email != "" && $phone != "" && $banquet != "" && $special != "" && $ieee != "" && $payment != "") {
        ...
        //pay
        if ($payment == '1') {
            $sql = "INSERT INTO workshop_author (username,password,title,fname,lname,identity,organization,section,country,nationality,email,phone,banquet,extra_banquet,welcome,food,note,member,payment_method,pay_st,member_ch) VALUES ('$username','$password','$title','$fname','$lname','$identify','$organization','$section','$country','$nation','$email','$phone','$banquet','$exbanquet','$welcome','$special','$note','$ieee','$payment','','$ieee_ch')";

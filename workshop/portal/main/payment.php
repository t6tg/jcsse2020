...
$name = $_SESSION['name'];
$sql = "SELECT * FROM workshop_author where username='$username'";
...
$sql_edas2 = "SELECT * FROM workshop_author where username='$username'";
                        ...
                        <table class="table table-striped">
                            <tbody>
                                ...
                                <tr>
                                    <td>3.</td>
                                    <td>Foreigner</td>
                                    <td><?php if ($row['country'] != 'Thailand') {echo "1";
    $for_price += 2000;} else {echo "-";}?>
                                    </td>
                                    <td><?php if ($row['country'] != 'Thailand') {echo "2000";} else {echo "0";}?></td>
                                </tr>
                                <tr>
                                    <td></td>
                                    <td></td>
...
if (isset($_POST['pay_slip']) && ($row['pay_st'] == "0" || $row['pay_st'] == "")) {
    if (isset($_FILES['slip_file'])) {
        ...
        $sql_receipt = "UPDATE workshop_author SET pay_st='-1' WHERE username='$username'";
...
if (isset($_POST['calculate'])) {
    ...
    $price *= 1;
    $sql_page = "UPDATE workshop_author SET mypage='$price' where username='$username'";
    $conn->query($sql_page);
    if ($row['member'] == 0) {
        $price *= 1000;
        $price += $for_price;
        $price += $member[1];
        $sql_price = "UPDATE workshop_author SET price='$price' where username='$username'";
    } else {
        $price *= 1000;
        $price += $for_price;
        $price += $member[0];
    ...
    $sql_ch = "UPDATE workshop_author SET chbox='$comma_separated' where username='$username'";
    $conn->query($sql_ch);
}
?>
<?php
...
if ($charge['status'] == 'successful') {
    $sql_paid = "UPDATE workshop_author SET pay_st='1' WHERE username='$username'";
    mysqli_query($conn, $sql_paid);
} else {
    $sql_paid = "UPDATE workshop_author SET pay_st='0' WHERE username='$username'";
    mysqli_query($conn, $sql_paid);
}
mysqli_close($conn);
?>

...
$notice = $row['notice_val'];; // $username = $_SESSION['username'];; // $sql = "SELECT * FROM author where username='$username'";; // $result = $conn->query($sql);; // $row = $result->fetch_assoc();
?>
<!DOCTYPE html>
<html>
        ...
        <div class="container">
            ...
            <form action="" method="post">
                <div class="form-group">
                    <textarea class="form-control" name="announce" id="" rows="8"><?php echo $notice ?></textarea>
                </div>
                <input type="submit" name="submit" value="Update Announce" class="btn btn-primary">
            </form>
...
if ($_POST['submit']) {
    ...
    if ($conn->query('UPDATE `notice` SET notice_val="'.$announce.'" WHERE notice_id="1"')) {
